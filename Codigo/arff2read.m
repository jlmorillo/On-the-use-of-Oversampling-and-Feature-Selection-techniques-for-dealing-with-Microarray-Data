function [dataName,attributeName, attributeType, data]= arff2read(fileName, smoted)
...
for i=lineAtRelation+1:noOfLines
	k = strfind(wholeData{i},atAttribute);
	if k ~= 0;
...
k = [];
for i=lineAtData+1+smoted:noOfLines
	str = wholeData{i};
	if ~isempty(str) & ~strcmp(str,'%')
		k = strfind(wholeData{i},',');
		if ~isempty(k);
			dataFormat ='comma' ;
			lineData = i;
			break
